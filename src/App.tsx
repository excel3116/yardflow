    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <form onSubmit={submit} className="w-full max-w-sm rounded-[8px] border border-[#2A323D] bg-[#14181E] p-5 shadow-2xl">
        ...
        <div className="text-[12px] text-[#8A93A3] mb-4 font-mono">
          {request.vehicle.vehicleNumber} · net weight from the second weighment
        </div>
        <label className="block text-[11px] uppercase tracking-wide text-[#6B7686] mb-1">Net weight (kg)</label>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <form onSubmit={submit} className="w-full max-w-sm rounded-[8px] border border-[#2A323D] bg-[#14181E] p-5 shadow-2xl">
        ...
        <div className="flex gap-2 mt-5">
          <button type="button" onClick={onClose} className="flex-1 rounded-[4px] border border-[#2A323D] py-2 text-sm text-[#8A93A3] hover:text-[#EDF1F5] hover:border-[#3A4451] transition-colors">Cancel</button>
          <button type="submit" className="flex-1 rounded-[4px] bg-[#4C8CF5] py-2 text-sm font-semibold text-[#08111F] hover:bg-[#659BF7] transition-colors">Assign</button>
        </div>
      </form>
    </div>
  );
...
function AssignYardModal({ request, onClose, onSubmit }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <form onSubmit={submit} className="w-full max-w-sm rounded-[8px] border border-[#2A323D] bg-[#14181E] p-5 shadow-2xl">
        ...
        <div className="text-[12px] text-[#8A93A3] mb-4 font-mono">
          {request.vehicle.vehicleNumber} · pick the yard for this truck before it arrives
        </div>
      ...
      <div className="w-full max-w-md h-full bg-[#12161C] border-l border-[#242B34] overflow-y-auto">
        ...
        <div className="p-5">
          ...
          {vehicle.netWeight != null && (() => {
            const dev = vehicle.partyNetWeight
              ? Math.abs(vehicle.netWeight - vehicle.partyNetWeight) / vehicle.partyNetWeight
              : null;
            return (
              <div className="grid grid-cols-2 gap-2 mb-5">
                <WeightReadout label="Party net wt. (declared)" value={vehicle.partyNetWeight} />
                <WeightReadout label="Net weight (measured)" value={vehicle.netWeight} highlight={dev != null && dev <= 0.02} warn={dev != null && dev > 0.02} />
              </div>
            );
          })()}
...
// ---------------------------------------------------------------------------
// Role queue view — what Security / Yard Supervisor / Yard Incharge / Weighbridge Operator / QC see (no dashboard)
// ---------------------------------------------------------------------------
...
const ACTION_LABELS = {
  ...
  "Yard Picked": "Assigned yard (pre-arrival)",
...
function Dashboard({ actualRole, profile, onLogout }) {
  ...
  const handleAdvance = useCallback(async (vehicle, action) => {
    ...
    if (action.type === "pickYard") {
      setYardAssignRequest({ vehicle, action });
      return;
    }
    ...
    if (action.type === "advance") {
      ...
      const isYardApproval = action.approverKey === "yardEntryApproved";
      const otherApproved = isYardApproval ? vehicle.securityReported : vehicle.yardEntryApproved;
      ...
      if (otherApproved) {
        update = {
          ...
          [isYardApproval ? "yard_entry_approved" : "security_reported"]: true,
  ...
  const handleWeighSubmit = useCallback(async (weightValue) => {
    ...
    const update = {
      status: action.next, status_at: new Date(at).toISOString(),
      net_weight: weightValue,
      history: [...(vehicle.history || []), { status: action.next, at, role: actor, outcome: "approved", note: `Net weight: ${weightValue.toLocaleString("en-IN")} kg` }],
      flagged: false, flag_reason: null, flagged_at: null,
    };
  ...
  const handleSupervisorSubmit = useCallback(async (supervisor) => {
    ...
    const update = {
      ...
      history: [...(vehicle.history || []), { status: "Supervisor Assigned", at, role: actor, outcome: "approved", note: `Assigned supervisor: ${supervisor}` }],
    };
  ...
  const handleAssignYardSubmit = useCallback(async (yard) => {
    if (!yardAssignRequest) return;
    const { vehicle, action } = yardAssignRequest;
    const at = Date.now();
    const actor = role === "Admin" ? action.role : role;
    const update = {
      yard,
      history: [...(vehicle.history || []), { status: "Yard Picked", at, role: actor, outcome: "approved", note: `Assigned yard: ${yard}` }],
    };

    const { data, error } = await supabase.from("vehicles").update(update).eq("id", vehicle.id).select();
    if (error) setConnectionError(error.message);
    else if (data && data[0]) setVehicles((vs) => vs.map((v) => (v.id === data[0].id ? rowToVehicle(data[0]) : v)));
    setYardAssignRequest(null);
    setSelectedVehicle(null);
  }, [yardAssignRequest, role]);
