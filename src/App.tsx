...
import {
  ...
  RotateCcw, Sparkles, MousePointerClick, Layers
} from "lucide-react";
...
const ROLES = [
  "Vendor", "Security", "Yard Supervisor", "Weighbridge Operator",
  "Loading Supervisor", "QC", "Admin",
];

const TRANSITIONS = {
  Expected: { role: "Security", label: "Vehicle Arrived", next: "Arrived", icon: ShieldCheck },
  Arrived: { role: "Yard Supervisor", label: "Assign Yard", next: "Yard Assigned", icon: Warehouse, needsYard: true },
  "Yard Assigned": { role: "Yard Supervisor", label: "Send to Weighbridge", next: "Sent to Weighbridge", icon: ArrowRight },
  "Sent to Weighbridge": { role: "Weighbridge Operator", label: "Start Weighment", next: "Weighed In", icon: Scale, weight: "gross" },
  "Weighed In": { role: "Loading Supervisor", label: "Loading Started", next: "Loading", icon: PackageCheck },
  Loading: { role: "Loading Supervisor", label: "Loading Complete", next: "Loading Complete", icon: PackageCheck },
  "Loading Complete": { role: "Weighbridge Operator", label: "Second Weighment", next: "Weighed Out", icon: Scale, weight: "tare" },
  "Weighed Out": { role: "QC", label: "Approve", next: "QC Approved", icon: CheckCircle2 },
  "QC Approved": { role: "Security", label: "Vehicle Out", next: "Exited", icon: LogOut },
};

const WAITING_STAGES = ["Yard Assigned", "Sent to Weighbridge"];
const VENDORS = ["Shree Cement Traders", "Balaji Logistics", "Om Sai Carriers", "Patel Freight Co", "Ganesh Roadlines"];
const MATERIALS = ["Cement", "Steel Coils", "Fly Ash", "River Sand", "Clinker", "Gypsum"];
const DESTINATIONS = ["Silo A", "Silo B", "Warehouse 3", "Plant Gate 2", "Stockyard 1"];
...
function statusColor(status) {
  ...
  if (status === "Exited") return { fg: "#3ECF8E", bg: "#122A22", bd: "#1E4A3A" };
...
function buildSeedRows() {
  ...
  return [
    ...
    seed(9, "Weighed Out", { yard: YARDS[4], gross_weight: 29800, tare_weight: 9600, net_weight: 20200 }),
  ];
}
  ...
  return (
    <button onClick={onClick}
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {(card.key === "WBWait" || card.key === "YardWait") && <Radio size={10} className="text-[#F2A93B] animate-pulse" />}
      </div>
...
const DASHBOARD_CARDS = [
  ...
  { key: "Inside", label: "Inside plant", icon: Truck, filter: (v) => v.status !== "Expected" && v.status !== "Exited" },
  { key: "YardWait", label: "Waiting in yard", icon: Warehouse, filter: (v) => v.status === "Yard Assigned" },
  { key: "WBWait", label: "Waiting for weighment", icon: Scale, filter: (v) => v.status === "Sent to Weighbridge" },
  { key: "Loading", label: "Loading in progress", icon: PackageCheck, filter: (v) => v.status === "Loading" },
  { key: "QCPending", label: "QC pending", icon: ClipboardList, filter: (v) => v.status === "Weighed Out" },
  { key: "Ready", label: "Ready for dispatch", icon: CheckCircle2, filter: (v) => v.status === "QC Approved" },
  { key: "ExitedToday", label: "Exited today", icon: LogOut, filter: (v) => v.status === "Exited" },
];

function AddVehicleModal({ onClose, onCreate }) {
  const [form, setForm] = useState({ vehicleNumber: "", driver: "", mobile: "", vendor: VENDORS[0], material: MATERIALS[0], po: "", destination: DESTINATIONS[0] });
  const update = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }));
  const submit = (e) => { e.preventDefault(); if (!form.vehicleNumber.trim()) return; onCreate(form); };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <form onSubmit={submit} className="w-full max-w-md rounded-[8px] border border-[#2A323D] bg-[#14181E] p-5 shadow-2xl">
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-[11px] uppercase tracking-wide text-[#6B7686] mb-1">Vehicle number *</label>
            <input autoFocus value={form.vehicleNumber} onChange={update("vehicleNumber")} placeholder="MH12AB1234"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] uppercase tracking-wide text-[#6B7686] mb-1">Driver name</label>
              <input value={form.driver} onChange={update("driver")} placeholder="Ramesh Yadav"
            ...
            <div>
              <label className="block text-[11px] uppercase tracking-wide text-[#6B7686] mb-1">Mobile</label>
              <input value={form.mobile} onChange={update("mobile")} placeholder="9812345678"
                className="w-full rounded-[4px] bg-[#1C222A] border border-[#2A323D] px-3 py-2 text-[#EDF1F5] font-mono text-sm focus:outline-none focus:border-[#4C8CF5]" />
            </div>
          </div>
          <div>
            <label className="block text-[11px] uppercase tracking-wide text-[#6B7686] mb-1">Vendor</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] uppercase tracking-wide text-[#6B7686] mb-1">Material</label>
            ...
            <div>
              <label className="block text-[11px] uppercase tracking-wide text-[#6B7686] mb-1">PO number</label>
              <input value={form.po} onChange={update("po")} placeholder="PO-48213"
                className="w-full rounded-[4px] bg-[#1C222A] border border-[#2A323D] px-3 py-2 text-[#EDF1F5] font-mono text-sm focus:outline-none focus:border-[#4C8CF5]" />
            </div>
          </div>
          <div>
            <label className="block text-[11px] uppercase tracking-wide text-[#6B7686] mb-1">Destination</label>
...
function WeightReadout({ label, value, highlight }) {
  return (
    <div className={`rounded-[4px] border px-2 py-2 text-center ${highlight ? "border-[#3ECF8E] bg-[#122A22]" : "border-[#2A323D] bg-[#1C222A]"}`}>
      <div className="text-[9px] uppercase tracking-wide text-[#6B7686]">{label}</div>
      <div className={`font-mono text-[14px] font-bold tabular-nums ${highlight ? "text-[#3ECF8E]" : "text-[#DCE2E8]"}`}>
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60">
      <div className="w-full max-w-md h-full bg-[#12161C] border-l border-[#242B34] overflow-y-auto">
        ...
        <div className="p-5">
            ...
            {(vehicle.grossWeight || vehicle.tareWeight) && (
              <>
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <WeightReadout label="Net" value={vehicle.netWeight} highlight />
                </div>
              </>
            )}
          </div>
          ...
          {canAct && (
            <button onClick={() => onAdvance(vehicle)}
          ...
          {!canAct && t && (
            <div className="mb-6 rounded-[6px] border border-[#2A323D] bg-[#161B22] px-3 py-2.5 text-[12px] text-[#6B7686]">
              Awaiting action from <span className="text-[#8A93A3] font-medium">{t.role}</span>
            </div>
          )}
...
function ReportsView({ vehicles }) {
  const metrics = [
    { label: "Avg. gate-to-yard time", value: avgBetween(vehicles, "Arrived", "Yard Assigned") },
    { label: "Avg. yard waiting time", value: avgBetween(vehicles, "Yard Assigned", "Sent to Weighbridge") },
    { label: "Avg. weighbridge wait", value: avgBetween(vehicles, "Sent to Weighbridge", "Weighed In") },
    { label: "Avg. loading duration", value: avgBetween(vehicles, "Loading", "Loading Complete") },
  ...
  return (
    <div>
      ...
      <div className="text-[11px] uppercase tracking-wide text-[#6B7686] mb-2">Vendor-wise activity</div>
          ...
          <tbody>
            {vendorStats.map(([vendor, s]) => (
              <tr key={vendor} className="border-t border-[#242B34]">
                ...
                <td className="px-4 py-2.5 text-right font-mono text-[#3ECF8E]">{s.exited}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
// ---------------------------------------------------------------------------
// Welcome overlay — shown once per browser so unaccompanied clients
// understand the concept before they start clicking around
// ---------------------------------------------------------------------------
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 px-4">
      <div className="w-full max-w-lg rounded-[10px] border border-[#2A323D] bg-[#14181E] p-6 shadow-2xl">
        ...
        <h2 className="font-[Barlow_Condensed] text-[26px] font-bold text-[#EDF1F5] tracking-wide mb-2">
          Welcome to YARDFLOW
        </h2>
        <p className="text-[13px] text-[#8A93A3] leading-relaxed mb-5">
          This is a live view of how a truck moves through your yard — from the moment it's expected,
          through weighment and loading, to exit. Everything here updates in real time, so if this is
          open on another screen too, you'll see changes appear instantly on both.
        </p>

        <div className="space-y-3 mb-6">
          <div className="flex gap-3 items-start">
            <div className="w-7 h-7 rounded-[6px] bg-[#122238] border border-[#1E3A5C] flex items-center justify-center shrink-0 mt-0.5">
              <User size={14} className="text-[#7CACF8]" />
            </div>
            <div className="text-[13px] text-[#DCE2E8]">
              <span className="font-semibold text-[#EDF1F5]">Switch roles</span> in the top-right dropdown —
              Security, Yard Supervisor, Weighbridge Operator, and so on — to see exactly what each person
              on your team would see and do.
            </div>
          </div>
          <div className="flex gap-3 items-start">
            <div className="w-7 h-7 rounded-[6px] bg-[#122238] border border-[#1E3A5C] flex items-center justify-center shrink-0 mt-0.5">
              <MousePointerClick size={14} className="text-[#7CACF8]" />
            </div>
            <div className="text-[13px] text-[#DCE2E8]">
              <span className="font-semibold text-[#EDF1F5]">Click any vehicle row</span> to see its full
              journey, weighment details, and gate-pass style summary.
            </div>
          </div>
          <div className="flex gap-3 items-start">
            <div className="w-7 h-7 rounded-[6px] bg-[#122238] border border-[#1E3A5C] flex items-center justify-center shrink-0 mt-0.5">
              <Layers size={14} className="text-[#7CACF8]" />
            </div>
            <div className="text-[13px] text-[#DCE2E8]">
              <span className="font-semibold text-[#EDF1F5]">Try the stat cards</span> at the top — click one
              to instantly filter the table below it.
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    try {
      if (!localStorage.getItem("yardflow_seen_intro")) {
        setShowWelcome(true);
      }
    } catch (e) {
      // localStorage unavailable — skip the one-time intro silently
    }
  }, []);
  ...
  const handleResetDemo = useCallback(async () => {
    ...
    if (existing && existing.length) {
      await supabase.from("vehicles").delete().in("id", existing.map((r) => r.id));
    }
  ...
  const handleAdvance = useCallback(async (vehicle) => {
    ...
    if (t.weight === "tare") {
      ...
      update.net_weight = (vehicle.grossWeight || randomBetween(28000, 33000)) - update.tare_weight;
    }
      ...
      <div className="border-b border-[#1C222A] bg-[#0E1116] sticky top-0 z-30">
        <div className="max-w-[1200px] mx-auto px-5 py-3 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="font-[Barlow_Condensed] text-[19px] font-bold leading-none tracking-wide">YARDFLOW</div>
              <div className="text-[10px] text-[#5A6270] leading-none mt-0.5 flex items-center gap-1">
                {connectionError ? (
                  <><WifiOff size={10} className="text-[#FF5C5C]" /> Connection issue</>
                ) : (
                  <><Wifi size={10} className="text-[#3ECF8E]" /> Live · connected</>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1.5 rounded-[6px] border border-[#242B34] bg-[#161B22] px-2.5 py-1.5">
              ...
              <select value={role} onChange={(e) => setRole(e.target.value)} className="bg-transparent text-[13px] text-[#EDF1F5] focus:outline-none">
            ...
            {role === "Admin" && (
              <button onClick={handleResetDemo} disabled={resetting}
                title="Restore the starting demo scenario"
      ...
      <div className="max-w-[1200px] mx-auto px-5 py-5">
        {connectionError && (
          <div className="mb-4 rounded-[6px] border border-[#4A3A1E] bg-[#2A2015] px-4 py-3 text-[13px] text-[#F2A93B]">
            Couldn't reach Supabase: {connectionError}. Check your project URL/key in supabaseClient.js.
          </div>
        )}
        {loading ? (
                    ...
                    return (
                      <tr key={v.id} onClick={() => setSelectedVehicle(v)} className="border-t border-[#242B34] hover:bg-[#151A20] cursor-pointer transition-colors">
                        ...
                        <td className="px-4 py-2.5 text-right">
                          {canAct ? (
                            <button onClick={(e) => { e.stopPropagation(); handleAdvance(v); }}
                            ...
                            <span className="text-[11px] text-[#5A6270]">{t ? `Awaiting ${t.role}` : "Completed"}</span>
                          )}
                        </td>
                      </tr>
                    );
